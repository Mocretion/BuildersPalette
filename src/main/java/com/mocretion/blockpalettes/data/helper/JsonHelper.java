...
public class JsonHelper {
    ...
    public static String jsonToString(JsonElement json, boolean withSignature){
        return withSignature ? BlockPalettes.MOD_NAME + GSON.toJson(json) : GSON.toJson(json);
    }

    public static JsonObject stringToJson(String jsonString, boolean onlyWithSignature){

        if(onlyWithSignature){
            if(jsonString.startsWith(BlockPalettes.MOD_NAME)){
                jsonString = jsonString.substring(BlockPalettes.MOD_NAME.length());
            }else{
                return null;
            }
        }
